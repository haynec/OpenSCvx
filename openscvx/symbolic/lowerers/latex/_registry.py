"""Visitor registry for the LaTeX lowerer.
...
_LATEX_VISITORS: Dict[Type[Expr], Callable] = {}
...
def visitor(expr_cls: Type[Expr]) -> Callable[[Callable], Callable]:
    ...
    type during LaTeX lowering. The decorated function is stored in
    _LATEX_VISITORS and will be called by dispatch() when lowering that
    expression type.
    ...
    Note:
        Dispatch is exact-type, so shared behaviour must be registered per
        concrete class.  Multiple types can share a visitor by stacking
        decorators::
    ...
    def register(fn: Callable[[Any, Expr], str]):
        _LATEX_VISITORS[expr_cls] = fn
        return fn

    return register


def dispatch(lowerer: Any, expr: Expr) -> str:
    ...
    Args:
        lowerer: The LatexLowerer instance (provides context for visitor methods)
        expr: The expression node to lower

    Returns:
        The result of calling the visitor function (a LaTeX string)

    Raises:
        NotImplementedError: If no visitor is registered for the expression type
    """
    fn = _LATEX_VISITORS.get(type(expr))
    if fn is None:
