...
def create_cvxpy_variables(
    ...
    lam_vb_nodal = cp.Parameter((N, n_nodal_constraints), nonneg=True, name="lam_vb_nodal")
    lam_vb_cross = cp.Parameter(n_cross_node_constraints, nonneg=True, name="lam_vb_cross")
    ...
    if len(constraints.nodal) > 0:
        ...
        for i, fn in enumerate(constraints_nodal_fns):
            ...
            constraint = LoweredNodalConstraint(
                ...
                lam_vb=constraints.nodal[i]._lam_vb[0] if constraints.nodal[i]._lam_vb else None,
            )
