...
(``examples/mpc/double_integrator_drone_racing.py``): phase 1 solves, once
per roster, the charge-sustaining minimum-time flying lap for *each car's own
spec* (power, mass, battery), and phase 2 races the field while every car
tracks its own lap. The phase-1 result is cached in
``race_car_reference.npz`` beside this file and re-solved automatically when
the roster or lane width changes.
...
Each error integrates into a tracking state (``track_n``/``track_v``/
``track_E``) whose final value is minimised, alongside a small residual
progress reward that gives cars a reason to overtake rather than follow. The
references enter as per-car Parameter tables interpolated by hat weights in
``s`` (the same idiom the opponent forecasts use in time), so every car
tracks its own lap and reference updates never recompile.
...
# ── Phase 1: reference laps (solved once, cached) ──────────────────────────────
...
# Cached beside this file; the cache is invalidated when the roster or lane
# changes, or regenerated by deleting it.
M_REF_LAP = 64
_REF_FILE = os.path.join(current_dir, "race_car_reference.npz")
_SPEC_MATRIX = np.array(
    [[spec[k] for k in ("power_scale", "mass_scale", "battery_scale")] for spec in AGENTS]
)


def _solve_reference_laps() -> dict:
    """Solve the batched per-spec reference laps and return the npz payload."""
    N = 80
    T_guess = 20.0
    ...
    ref_problem = ox.Problem(
        ...
        licq_max=1e-12,
        algorithm={
            # The charge-sustaining lap destabilizes under warm-start
            # continuation with a looser anchor; this keeps every round finite
            # while still escaping the initial guess.
            "lam_prox": 1e-1,
            "lam_cost": 3e0,
            "lam_vc": 1e2,
    ...
    return dict(
        ...
        spec_matrix=_SPEC_MATRIX,
        lane_half_width=np.float64(LANE_HALF_WIDTH),
        **tables,
    )


def _load_or_solve_reference():
    if os.path.exists(_REF_FILE):
        cache = np.load(_REF_FILE)
        if (
            "spec_matrix" in cache.files
            and cache["spec_matrix"].shape == _SPEC_MATRIX.shape
            and np.allclose(cache["spec_matrix"], _SPEC_MATRIX)
            and float(cache["lane_half_width"]) == LANE_HALF_WIDTH
        ):
            return cache
    print("Reference cache missing or stale — solving the reference laps (a few minutes)...")
    np.savez(_REF_FILE, **_solve_reference_laps())
    return np.load(_REF_FILE)


DRIVER_STATES = ("s", "n", "alpha", "v", "D", "delta", "E", "R")

_ref = _load_or_solve_reference()
...
# Tracking cost integrators: each accumulates a squared tracking error along
# the horizon and is minimised at the final node (the MPCC lag_sum/contour_sum
# idiom). Box bounds only set solver scaling; the states are never constrained.
track_n = ox.State("track_n", shape=(1,))
track_n.min, track_n.max = [0.0], [0.5]
track_n.initial, track_n.final = [0.0], [ox.Minimize(0.0)]
track_n.guess = np.zeros((N_MPC, 1))

track_v = ox.State("track_v", shape=(1,))
track_v.min, track_v.max = [0.0], [20.0]
track_v.initial, track_v.final = [0.0], [ox.Minimize(0.0)]
track_v.guess = np.zeros((N_MPC, 1))

track_E = ox.State("track_E", shape=(1,))
track_E.min, track_E.max = [0.0], [0.05]
...
# ── Problem ────────────────────────────────────────────────────────────────────
# Tracking weights: the references carry the racing line, pace, and energy
# plan, so the progress reward drops to a residual tie-breaker and the cost is
# dominated by regulation around the reference.
W_PROGRESS = 3e1
W_TRACK_N = 3e1
W_TRACK_V = 1e1
W_TRACK_E = 1e2
