"""Multi-agent tracking race, pure-combustion ablation of the MPCC example.

Same two-phase structure, field, track, collision scheme, and tracking
objective as ``race_car_multi_agent_mpcc.py``, with the hybrid power unit
removed: no battery state, no recovery accounting, no deploy/harvest
controls, and no energy reference to track. The full Kloeser drive envelope
(Cm1 - Cm2·v)·D goes through the throttle alone, so D = -1 is full-strength
friction braking.

The ablation is numerical, not sporting. Per car the hybrid variant carries
two extra driver states (E, R), one extra tracking integrator (track_E), and
two extra FOH controls (deploy, regen) — all purely for energy strategy.
Removing them shrinks every horizon subproblem and its Jacobians, so this
file measures how much of the MPC step time and convergence behaviour the
energy management costs on top of the racing itself:

  * phase 1 solves each spec's minimum-time flying lap (periodic driving
    states — with no battery there is nothing else to sustain),
  * phase 2 races the field while every car regulates around the nominal
    lap through the same shared pchip reference splines and per-car pace
    scalar, tracking only the racing line and the speed profile.

!!! note "Twin example"
    ``race_car_multi_agent_mpcc.py`` is the full hybrid version; this file
    differs only in the power unit. The roster is identical (the specs vary
    power and mass, not battery).

!!! warning "Not real time"
    This is an offline closed-loop simulation: each 0.1 s MPC step takes a
    fraction of a second to solve on a laptop CPU — growing with the size of
    the field — and phase 1 solves the reference laps once up front.
...
# Each car's minimum-time flying lap for its own spec, under the race bounds:
# periodic driving states (with no battery there is nothing else to sustain).
# The result is sampled on a uniform s grid (coarse is fine — the profiles are
...
def _solve_reference_laps() -> dict:
    ...
    rstates = [rs, rn, ralpha, rv, rD, rdelta]
    rcontrols = [rderD, rderDelta]
    ...
    # Flying-lap periodicity on the driving states.
    rconstraints.extend((x.at(0) == x.at(N - 1)).convex() for x in [rn, ralpha, rv, rD, rdelta])
    ra_lat = (
    ...
    specs = {key: np.array([spec[key] for spec in AGENTS]) for key in ("power_scale", "mass_scale")}
    ...
    signals = {
        "ref_n": (cols["n"], (-n_margin, n_margin)),
        "ref_v": (cols["v"], (0.0, 6.0)),
        "ref_D": (cols["D"], (-1.0, 1.0)),
    }
    tables = {sig: np.zeros((K, M_REF_LAP)) for sig in signals}
    for i in range(K):
        order = np.argsort(x_full[i, :, cols["s"]])
        s_o = x_full[i, order, cols["s"]]
        for sig, (col, (lo, hi)) in signals.items():
            tables[sig][i] = np.clip(np.interp(s_grid, s_o, x_full[i, order, col]), lo, hi)
...
DRIVER_STATES = ("s", "n", "alpha", "v", "D", "delta")
...
REF_IDX = next(
    (i for i, spec in enumerate(AGENTS) if spec["power_scale"] == spec["mass_scale"] == 1.0),
    0,
)
...
# car-width off line, ~1 m/s off pace): going off-reference to pass must sit
# comfortably inside the cap, because an integrator that rails its box fights
# the solver on every horizon it does.
...
# ── Dynamics (spatial bicycle model, per-car spec via parameters) ──────────────
...
# ── Constraints ────────────────────────────────────────────────────────────────
states = [s, n, alpha, v, D_throt, delta, track_n, track_v]
controls = [derD, derDelta]

constraints: list = []

for state in [s, n, alpha, v, D_throt, delta]:
    constraints.extend(
        [
            ox.ctcs(state <= state.max, penalty="huber"),
            ox.ctcs(state.min <= state, penalty="huber"),
...
# ── Problem ────────────────────────────────────────────────────────────────────
# The reference carries the racing line and pace, so the cost is regulation
# around it plus a residual progress reward that gives cars a reason to
# overtake rather than follow. ``lam_cost`` weighs the *scaled* integrators —
# state scaling divides by max(1, half-range of the box), so a weight's
# physical strength is its lam over that factor, and the narrow track_n box
# clamps to 1 (its lam IS its strength). Contour is priced cheapest: lateral
# room is the overtaking degree of freedom, and racing thrashes (convergence
# and solve time both suffer) when the line or pace is too dear to leave —
# but on a shared line it cannot be too cheap either, or side-by-side duels
# stop resolving. Weights carried over unchanged from the hybrid twin (minus
# the energy term) so the ablation isolates the model change.
W_PROGRESS = 3e1
W_TRACK_N = 5e0
W_TRACK_V = 9e1
...
def initial_pins() -> np.ndarray:
    """(K, n_x) boundary pins: every car parked in its grid slot."""
...
def shifted_guesses(results) -> tuple[np.ndarray, np.ndarray]:
    ...
    s_tip = tail[:, 0, COL["s"]]
    for name, table in [("n", REF_N), ("v", REF_V), ("D", REF_D)]:
...
def accelerations(x: np.ndarray, spec: dict) -> tuple[np.ndarray, np.ndarray]:
    ...
    Fxd = F_env * D - Cr2 * v**2 - Cr0 * np.tanh(5.0 * v)
...
def plot_race(log: RaceLog) -> None:
    """Track projection and speed stripline.

    As in ``race_car_multi_agent.plot_race``, everything is drawn from the
    dense propagated log against race distance in laps, with the dotted shared
    reference on the speed stripline: the nominal lap every car tracks, tiled
    over the race, so the tracking quality — and any deviation bought to race
    ...
    fig1 = track_figure(
        ...
        LANE_HALF_WIDTH,
        title=(
            "Tracking race (pure ICE) — propagated closed-loop trajectories  "
            f"({len(log.sim)} steps)"
        ),
    )
    ...
    fig2.update_layout(
        title="Speed vs the shared reference lap",
...
def build_viser_panels(log: RaceLog) -> list[dict]:
    """Live plot panels for the race replay: speed and g-g.
    ...
    gg_car = [accelerations(log.dense_x[i, : end[i]], spec) for i, spec in enumerate(AGENTS)]
...
def run_race(max_steps: int = MAX_STEPS) -> RaceLog:
    ...
    fixed_params = {
        key: np.array([spec[key] for spec in AGENTS]) for key in ("power_scale", "mass_scale")
    }
    ...
    for step in range(max_steps):
        ...
        dense_u.append(np.asarray(post.u_full)[:, keep, :2])
        ...
        # (e.g. running wide of the lane on an unconverged plan), but an
        # x_initial pinned outside the box cannot satisfy the subproblem's node
        # bounds and poisons that car's solve, so the plant enforces the box at
        # the handoff.
...
def print_classification(log: RaceLog) -> list[int]:
    """Print the final order; returns the order."""
