...
REF_IDX = next(
    (
        ...
        if spec["power_scale"] == spec["mass_scale"] == 1.0
    ),
    0,
)
...
def plot_race(log: RaceLog) -> None:
    ...
    fig1.update_layout(
        title=f"Tracking race (pure ICE) — propagated closed-loop trajectories  ({len(log.sim)} steps)",
...
def build_viser_panels(log: RaceLog) -> list[dict]:
    ...
    to the cars' state at race time ``t``. The stripline runs against race
