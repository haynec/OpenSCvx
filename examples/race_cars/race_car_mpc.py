...
def plot_mpc_results(
    ...
    for snap in horizon_snapshots:
        ...
        fig1.add_trace(go.Scatter(
            ...
            showlegend=first, legendgroup="horizon",
        ))
        first = False
    ...
    fig1.add_trace(go.Scatter(
        ...
        marker=dict(
            color=simX[:, 3], colorscale="Rainbow", size=4,
    ...
    for i in range(int(sref_d[-1]) + 1):
        ...
        fig1.add_annotation(
            x=xref_d[k], y=yref_d[k], text=f"{i}m",
            showarrow=False, font=dict(size=10),
        )

    fig1.update_layout(
        title=f"OpenSCvx MPC — track projection  ({len(simX)} steps, "
              f"T={t_vec[-1]:.2f} s)",
    ...
    fig2 = make_subplots(
        rows=n_states + 2, cols=1, shared_xaxes=True,
    ...
    for i in range(n_states):
        fig2.add_trace(go.Scatter(
            x=t_vec, y=simX[:, i], mode="lines",
            line=dict(color=colors[i]), name=labels_x[i], showlegend=False,
        ), row=i + 1, col=1)
    for i, name in enumerate(labels_u):
        fig2.add_trace(go.Scatter(
            x=t_vec, y=simU[:, i], mode="lines",
            name=name, showlegend=False,
        ), row=n_states + i + 1, col=1)
    ...
    fig3.update_layout(
        ...
        xaxis_title="t [s]", yaxis_title="acceleration [m/s²]",
        height=400,
    )
    fig3.show()
...
if __name__ == "__main__":
    ...
    for step in range(Nsim_max):
        ...
        simX[step] = [
            nodes["s"][0, 0], nodes["n"][0, 0], nodes["alpha"][0, 0],
            nodes["v"][0, 0], nodes["D"][0, 0],  nodes["delta"][0, 0],
        ]
        ...
        if step % 5 == 0:
            ...
            horizon_snapshots.append(np.column_stack([
                traj["s"][:, 0], traj["n"][:, 0],
                traj["alpha"][:, 0], traj["v"][:, 0],
            ]))
