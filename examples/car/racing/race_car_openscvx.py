...
Ports the acados NMPC benchmark from:
  Daniel Kloeser, Tobias Schoels, Tommaso Sartor, Andrea Zanelli, Gianluca
  Frison, Moritz Diehl, "NMPC for Racing Using a Singularity-Free
  Path-Parametric Model with Obstacle Avoidance", Proceedings of the 21st
  IFAC World Congress, Berlin, July 2020.
  https://www.youtube.com/watch?v=1JDBQXVrZbo

This file is the baseline of the racing family: every example in this
directory races the same spatial bicycle model on the same LMS kart track,
and each later file adds exactly one idea — a hybrid power unit
(``race_car_hybrid.py`` and its pure-combustion twin ``race_car_ice.py``),
a receding horizon (``race_car_mpc.py``), a field of opponents
(``race_car_multi_agent.py``), and reference tracking for the whole field
(``race_car_multi_agent_mpcc.py`` / ``_mpcc_ice.py``). The path/Cartesian
conversion (``_time2spatial.py``) and track-table loader
(``_tracks/readDataFcn.py``) are vendored acados helpers (2-clause BSD);
apart from the underscore prefix their upstream names are kept deliberately.
...
def plot_race_results(results) -> None:
    ...
    from examples.car.racing._plotting import acceleration_figure, track_figure
