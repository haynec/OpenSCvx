"""
6-DoF Powered Descent Guidance with compound STCs via ``ox.stl.IfThen``

Same physics, boundaries, and trigger thresholds as ``6DoF_pdg_stc.py``, but
the state-triggered rules are written as STL implications instead of hand-rolled
ReLU products.

How the STCs are encoded
------------------------
Each rule is an implication ``IfThen(trigger, consequent)`` with ``lite=True``
so the GMSR residual integrates cleanly under CTCS. Compound triggers use
``ox.stl.And`` / ``ox.stl.Or``:

    IfThen(h < 100 m, |δ_e| ≤ 1°)
    IfThen(And(h < 100 m, h > 2 m), tight glideslope)
    IfThen(And(||v|| < 35, tilt < 60°), single-engine thrust)
    IfThen(Or(||v|| > 35, tilt > 60°), three-engine thrust)
...
Sibling of ``6DoF_pdg_stc.py`` (custom product encoding). Reference:
CT-cSTC/CT-cSTC.ipynb
...
attitude.initial = [
    ox.Free(float(Q_INIT[0])),
    ox.Free(float(Q_INIT[1])),
    ox.Free(float(Q_INIT[2])),
    ox.Free(float(Q_INIT[3])),
]
...
C_aero_ox = ox.Diag(ox.Concat(float(C_AERO_DIAG[0]), float(C_AERO_DIAG[1]), float(C_AERO_DIAG[2])))
...
r_dot_los = pos_rel_x * los_I[0] + pos_rel_y * los_I[1] + pos_rel_z * los_I[2]


# ── Trigger / consequent predicates (Constraint objects for IfThen) ───────────
# Triggers (antecedents)
alt_lt_100 = z_alt <= _alt_h1_s  # h < 100 m
alt_lt_110 = z_alt <= _alt_h1a_s  # h < 110 m (alpha-tightened)
alt_lt_220 = z_alt <= _alt_h2a_s  # h < 220 m (LOS, alpha)
alt_gt_2 = z_alt >= _alt_done_s  # h >   2 m (finalizer)
v_lt_35 = speed <= _spd_trig_s  # ||v|| < 35 m/s
v_gt_35 = speed >= _spd_trig_s  # ||v|| > 35 m/s
tilt_lt_60 = tilt_sq <= _tilt_sq_trig  # tilt < 60°
tilt_gt_60 = tilt_sq >= _tilt_sq_trig  # tilt > 60°

# GMSR smoothing shared by all STC predicates (lite for CTCS integration)
_STC_C = 1e-12
_STC_KW = dict(c=_STC_C, lite=True)
_STC_OVER = dict(penalty="huber")

# Consequents
gimbal_tight = ox.stl.And(
    de <= _delta_stc_rad,
    -de <= _delta_stc_rad,
    **_STC_KW,
)  # |δ_e| ≤ 1°
gs_tight = r_xy_norm * _tan_gs_stc - pos_rel_z <= 0  # tight glideslope
omega_tight = omega_sq <= _omega_sq_stc  # tight angular rate
tilt_tight = tilt_sq <= _tilt_sq_stc  # tight tilt
spd_tight = speed <= _v_stc_s  # tight speed
los_tight = r_los_norm * _cos_psi_stc - r_dot_los <= 0  # LOS cone
thrust_aft = ox.stl.And(
    T <= _T_max_aft_s,
    T >= _T_min_aft_s,
    **_STC_KW,
)  # single-engine band
thrust_multi = ox.stl.And(
    T <= _T_max_s,
    T >= _ALPHA_T_MIN * _T_min_s,
    **_STC_KW,
)  # three-engine band

# Compound triggers
alt100_and_above_pad = ox.stl.And(alt_lt_100, alt_gt_2, **_STC_KW)
alt220_and_above_pad = ox.stl.And(alt_lt_220, alt_gt_2, **_STC_KW)
aft_phase = ox.stl.And(v_lt_35, tilt_lt_60, **_STC_KW)
multi_phase = ox.stl.Or(v_gt_35, tilt_gt_60, **_STC_KW)
...
# hand-rolled ReLU products in 6DoF_pdg_stc.py. Small c → tight Boolean approx.

# h < 100 m → tight gimbal / tilt
constraints.append(ox.stl.IfThen(alt_lt_100, gimbal_tight, **_STC_KW).over((0, N - 1), **_STC_OVER))
constraints.append(ox.stl.IfThen(alt_lt_100, tilt_tight, **_STC_KW).over((0, N - 1), **_STC_OVER))
# h < 100 m AND h > 2 m → tight glideslope cone
constraints.append(
    ox.stl.IfThen(alt100_and_above_pad, gs_tight, **_STC_KW).over((0, N - 1), **_STC_OVER)
)
# h < 110 m → tight angular rate and tight speed
constraints.append(ox.stl.IfThen(alt_lt_110, omega_tight, **_STC_KW).over((0, N - 1), **_STC_OVER))
constraints.append(ox.stl.IfThen(alt_lt_110, spd_tight, **_STC_KW).over((0, N - 1), **_STC_OVER))
# h < 220 m AND h > 2 m → LOS boresight cone toward R_I_APEX
constraints.append(
    ox.stl.IfThen(alt220_and_above_pad, los_tight, **_STC_KW).over((0, N - 1), **_STC_OVER)
)

# ||v|| < 35 m/s AND tilt < 60° → single-engine thrust limits
constraints.append(ox.stl.IfThen(aft_phase, thrust_aft, **_STC_KW).over((0, N - 1), **_STC_OVER))
# ||v|| > 35 m/s OR tilt > 60° → three-engine thrust limits
constraints.append(
    ox.stl.IfThen(multi_phase, thrust_multi, **_STC_KW).over((0, N - 1), **_STC_OVER)
)
...
time = ox.Time(
    initial=0.0, final=ox.Free(_t_f_guess), min=0.0, max=_t_f_guess * 1.5, uniform_time_grid=True
)
