...
constraints.append(
    ox.stl.IfThen(alt_lt_100, gimbal_tight, **_STC_KW).over((0, N - 1), **_STC_OVER)
)
constraints.append(
    ox.stl.IfThen(alt_lt_100, tilt_tight, **_STC_KW).over((0, N - 1), **_STC_OVER)
)
...
constraints.append(
    ox.stl.IfThen(alt_lt_110, omega_tight, **_STC_KW).over((0, N - 1), **_STC_OVER)
)
constraints.append(
    ox.stl.IfThen(alt_lt_110, spd_tight, **_STC_KW).over((0, N - 1), **_STC_OVER)
)
...
# ||v|| < 35 m/s AND tilt < 60° → single-engine thrust limits
constraints.append(
    ox.stl.IfThen(aft_phase, thrust_aft, **_STC_KW).over((0, N - 1), **_STC_OVER)
)
# ||v|| > 35 m/s OR tilt > 60° → three-engine thrust limits
constraints.append(
