...
def create_sensor_fpv_server(
    ...
    *,
    ...
    scene_scale: float,
    decorate_scene: Callable[[viser.ViserServer], UpdateCallback | None] | None = None,
    fov_deg: float = 76.0,
) -> viser.ViserServer:
    ...
    scenery the example wants (terrain, in the SENSS variants).  Scenery that
    animates — a LiDAR point cloud swept along the trajectory, say — returns its
    update callback, which is driven alongside the camera.
    """
    traj = result.trajectory
    ...
    update_scenery = decorate_scene(server) if decorate_scene is not None else None
    ...
    callbacks = [update_camera] if update_scenery is None else [update_camera, update_scenery]
