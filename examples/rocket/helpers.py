...
def orbital_elements_2_cartesian_rv(orbital_elements, gravitational_parameter):
    ...
    semimajor, eccentricity, inclination, right_ascension, arg_periapsis, true_anomaly = orbital_elements
    ...
    v_vect  = np.sqrt(gravitational_parameter / p) * np.array([-np.sin(true_anomaly), eccentricity + np.cos(true_anomaly), 0.0])
    ...
    R = np.array([
        [cos_Om*cos_om - sin_Om*sin_om*cos_i, -cos_Om*sin_om - sin_Om*cos_om*cos_i,  sin_Om*sin_i],
        [sin_Om*cos_om + cos_Om*sin_om*cos_i, -sin_Om*sin_om + cos_Om*cos_om*cos_i, -cos_Om*sin_i],
...
def cartesian_rv_2_orbital_elements(r_vect, v_vect, gravitational_parameter):
    ...
    e_vect          = (1.0 / gravitational_parameter) * ((v_norm_sq - gravitational_parameter / r_norm) * r_vect - (r_vect @ v_vect) * v_vect)
    ...
    return np.array([semimajor, eccentricity, inclination, right_ascension, arg_periapsis, true_anomaly])
...
if __name__ == "__main__":
    ...
    list_names = ["semimajor", "eccentricity", "inclination", "right_ascension", "arg_periapsis", "true_anomaly"]
    for i in range( len(list_names) ):
        print([list_names[i], f"orbital_in:{orbital_elements[i]:.3f}", f"orbital_out:{orbital_elements_out[i]:.3f}"])

    for i in range( len( orbital_elements_out ) ):
        assert np.linalg.norm( orbital_elements_out[i] - orbital_elements[i] ) <= np.sqrt( np.finfo("float").eps )
