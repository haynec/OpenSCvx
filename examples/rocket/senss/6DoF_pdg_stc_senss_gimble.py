...
def export_trajectory_rpy_csv(
    ...
    *,
    ...
    ``slowdown`` stretches wall-clock time (1 = real-time, 2 = 2× slower, etc.) and
    inserts linearly interpolated samples between each original pair so the exported
    sampling rate matches the original.
    """
    if not isinstance(slowdown, int) or slowdown < 1:
        raise ValueError(f"slowdown must be a positive integer >= 1, got {slowdown}")
    traj = result.trajectory
    ...
    if slowdown > 1:
        ...
        t_dense = np.concatenate(
            [np.linspace(t[i], t[i + 1], slowdown, endpoint=False) for i in range(n - 1)]
            + [t[-1:]]
        )
        pos = np.column_stack([np.interp(t_dense, t, pos[:, j]) for j in range(pos.shape[1])])
        rpy = np.column_stack([np.interp(t_dense, t, rpy[:, j]) for j in range(rpy.shape[1])])
        t = t_dense
