...
Identical to ``6DoF_pdg_stc_senss.py`` except: the initial and terminal
states lie in the x = 0 plane, ``DELTA_ENGINE_MAX_DEG`` 60° → 10°,
``THETA_MAX_DEG`` 10° → 20°, no aerodynamics, and the SENSS planar-rig CSV
convention (swapped horizontal axes, small roll/pitch trims).
...
# Initial conditions
R_I_INIT = np.array([0.0, -200.0, 250.0])  # m
V_I_INIT = np.array([0.0, 30.0, -10.0])  # m/s
...
# Terminal conditions
R_I_FINAL = np.array([0.0, -125.0, -200.0])  # m — touchdown target (slightly above pad)
...
THETA_MAX_DEG = 20.0  # max tilt from vertical
...
def export_trajectory_rpy_csv(
    ...
    *,
    ...
    """Write the trajectory in the SENSS planar rig's frame.
    ...
    small constant offsets trim the rig's mechanical zero — see the report
    accompanying this example set before changing them.
    """
