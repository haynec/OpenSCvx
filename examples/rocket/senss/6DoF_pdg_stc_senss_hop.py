...
Differs from ``6DoF_pdg_stc_senss.py`` in the boundary geometry — a
three-waypoint hop rather than a descent, so ``N`` doubles to 60 and the
landing-pad geometry is restricted to the descent half — plus no
aerodynamics and the SENSS platform CSV axis convention. Same cSTC
encoding and vehicle model throughout:
...
DEM_PLACEMENT = DemPlacement(
    origin_m=(39.0, 75.0, -244.0),
    scale_xyz=(0.65, 0.65, 0.7),
    grid=NATIVE_GRID,
)
...
def export_trajectory_rpy_csv(
    ...
    *,
    ...
    """Write the trajectory in the SENSS motion-platform frame.

    Positions are the solver's scaled coordinates (not multiplied by
    ``R_SCALE``), with the rig's x/y axes taken from the model's y and -x.
    Unlike the descent variants this hop exports roll and pitch unmodified —
    the rig is driven from its own zero here, with no mechanical trim applied.
    """
