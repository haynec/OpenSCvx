...
This is the canonical SENSS variant; ``_gimbal``, ``_planar``, ``_hop``, and
``_gimbal_hop`` differ from it only in their initial/terminal geometry, a
handful of angle limits, whether aerodynamics are modelled, and the CSV axis
convention their motion platform expects.
...
# Drag enters the moment balance below (via the centre-of-pressure lever arm)
# but is deliberately left out of the translational dynamics; ``6DoF_pdg_stc.py``
# keeps both terms.
...
DEM_PLACEMENT = DemPlacement(origin_m=(-78.0, -72.0, -244.0), grid=2048)
...
def export_trajectory_rpy_csv(
    ...
    *,
    ...
    ``R_SCALE``).  The platform's roll and pitch axes are the model's pitch and
    negated roll, and the small constant offsets trim its mechanical zero —
    see the report accompanying this example set before changing them.
    """
    ...
    return write_trajectory_rpy_csv(
        path,
        np.asarray(traj["time"], dtype=np.float64),
