...
Identical to ``6DoF_pdg_stc_senss_hop.py`` except: the pads sit closer
together at a higher elevation, the apex waypoint pins all three position
components rather than altitude alone, ``DELTA_ENGINE_MAX_DEG`` 60° → 10°,
``THETA_MAX_DEG`` 10° → 90°, and the CSV carries the gimbal rig's −90°
pitch trim. Same cSTC encoding and vehicle model:
...
When run as a script, launches four viser windows after solving:
  ...
  2. Onboard sensor FPV – camera locked to the LOS gimbal sensor with
     matched DEM placement, adjustable sensor FOV and XYZ position offset
     sliders, and the same LiDAR overlay
...
from examples.rocket.senss._dem import (
    ...
    dem_info_markdown,
    load_senss_dem,
    terrain_param_getter,
)
from examples.rocket.senss._hop_lidar_overlay import add_body_fixed_lidar_overlay
...
R_I_LAUNCH = np.array([-10.0, -125.0, -200.0])  # m — start on pad
R_I_APEX = np.array([200.0, -200.0, 250.0])  # m — mid-hop waypoint
R_I_LAND = np.array([+10.0, -100.0, -200.0])  # m — nearby land pad (same elevation)
...
_pos_guess[: K_APEX + 1] = (1.0 - _tau_asc)[:, None] * _r_launch_s + _tau_asc[:, None] * _r_apex_s
_pos_guess[K_APEX:] = (1.0 - _tau_des)[:, None] * _r_apex_s + _tau_des[:, None] * _r_land_s
...
r_dot_los = pos_los_x * los_I[0] + pos_los_y * los_I[1] + pos_los_z * los_I[2]
...
constraints.append((position == _r_apex_s).convex().at([K_APEX]))  # position only
...
constraints.append(
    ox.ctcs(r_xy_land * _tan_gs_max - pos_rel_z <= 0, penalty="huber").over((K_APEX, N - 1))
)
...
# then paste the values you settle on back here.  These values are also what
# puts the body-fixed LiDAR returns on the map, so retune the two together.
# The coarse grid keeps the LiDAR sweep interactive; raise it for stills.
DEM_PLACEMENT = DemPlacement(origin_m=(39.0, -175.0, -244.0), grid=300)

SCALES = CstcScales(r_scale=R_SCALE, m_scale=M_SCALE)
LIMITS = CstcLimits(
    gs_apex_m=R_I_LAND,
    los_apex_m=R_I_LOS_LAND,
    pad_z_m=float(R_I_LAUNCH[2]),
...
# Discs are centred between the two pads, which share an elevation.
_PAD_MIDPOINT = 0.5 * (R_I_LAUNCH + R_I_LAND)

SITE_MARKERS = [
    ("/los_target_land", R_I_LOS_LAND, (255, 220, 80), 0.08),
    ("/launch_pad", R_I_LAUNCH, (80, 180, 255), 0.12),
    ("/landing_pad", R_I_LAND, (50, 255, 80), 0.12),
]
ALT_TRIGGERS = [
...
def add_terrain(server, *, ambient: float = 0.0) -> Callable[[], dict]:
    """Overlay the DEM patch and a hop-geometry summary onto ``server``.
    ...
    Returns the live terrain-parameter getter, so the body-fixed LiDAR overlay
    samples the patch where the GUI sliders currently put it.
    """
    live_placement = add_dem_terrain(
        server,
    ...
    return terrain_param_getter(live_placement)
...
def launch_viser_servers(result) -> None:
    ...
    get_terrain_params = add_terrain(traj_server)
    add_camera_fov_slider(traj_server, initial_deg=CAMERA_FOV_DEG)

    # Post-process only: the LiDAR samples the same DEM the scene draws, through
    # the live getter, so dragging the terrain sliders moves the returns with it.
    update_lidar = add_body_fixed_lidar_overlay(
        ...
        scene_scale=SCENE_SCALE,
        dem_norm=load_senss_dem(DEM_PLACEMENT.grid),
        dem_center_norm=dem_center_norm(DEM_PLACEMENT.grid),
        get_terrain_params=get_terrain_params,
        terrain_half_extent_m=DEM_PLACEMENT.half_extent_m,
        base_relief_m=DEM_PLACEMENT.base_relief_m,
        name="/lidar_pcd",
    ...
    add_animation_controls(
        traj_server,
        handle.traj_time,
        handle.update_callbacks + [update_viewcone, update_lidar],
        loop=True,
    )

    # Both glideslope cones stand on the *landing* pad — the ascent leg is not
    # constrained by them — so each needs its own scene path.
    ...
    add_glideslope_cone(
        ...
        color=(80, 200, 80),
        opacity=0.10,
        name="/constraints/glideslope_wide_land",
    )
    add_glideslope_cone(
        traj_server,
        apex=land_vis,
        ...
        color=(255, 180, 40),
        opacity=0.14,
        name="/constraints/glideslope_tight_land",
    )

    add_altitude_trigger_discs(
        traj_server,
        pos,
        center_xy=(_PAD_MIDPOINT[0], _PAD_MIDPOINT[1]),
        base_z_m=float(R_I_LAUNCH[2]),
        scene_scale=SCENE_SCALE,
        triggers=ALT_TRIGGERS,
    )
    add_site_markers(
        traj_server,
        [*SITE_MARKERS, ("/apex_waypoint", R_I_APEX, (255, 140, 40), 0.14)],
        scene_scale=SCENE_SCALE,
    )
    ...
    def decorate_fpv(server):
        """Terrain plus the same body-fixed LiDAR sweep, seen down the boresight."""
        return add_body_fixed_lidar_overlay(
            server,
            pos_vis=np.asarray(result.trajectory["position"], dtype=np.float64),
            attitude_wxyz=np.asarray(result.trajectory["attitude"], dtype=np.float64),
            scene_scale=SCENE_SCALE,
            dem_norm=load_senss_dem(DEM_PLACEMENT.grid),
            dem_center_norm=dem_center_norm(DEM_PLACEMENT.grid),
            get_terrain_params=add_terrain(server, ambient=0.45),
            terrain_half_extent_m=DEM_PLACEMENT.half_extent_m,
            base_relief_m=DEM_PLACEMENT.base_relief_m,
            name="/lidar_pcd_fpv",
            gui_folder="LiDAR (body-fixed)",
...
def export_trajectory_rpy_csv(
    ...
    *,
    ...
    this example set before changing that offset.
    """
    ...
    return write_trajectory_rpy_csv(path, np.asarray(traj["time"], dtype=np.float64), pos, rpy)
...
def plot_cstc_results(result, *, show: bool = True, save_prefix: str = "cstc_hop") -> tuple:
    ...
    fig_3d = plot_cstc_trajectory_3d(
        ...
        scales=SCALES,
        markers=[
            ("Launch pad", R_I_LAUNCH, "cyan"),
            ("Apex", R_I_APEX, "orange"),
            ("Landing pad", R_I_LAND, "lime"),
        ],
        title="6-DoF cSTC Hop Trajectory",
    )
