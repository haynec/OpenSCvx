...
def add_dem_terrain(
    ...
    *,
    ...
    Returns a getter for the *current* placement, so that anything sampling the
    same terrain — the body-fixed LiDAR overlay, for one — follows the sliders
    instead of drifting off the mesh the moment the patch is nudged.
    """
    ...
    def live_placement() -> DemPlacement:
        with state["lock"]:
            return state["placement"]

    return live_placement


def terrain_param_getter(
    live_placement: Callable[[], DemPlacement],
) -> Callable[[], dict]:
    ...
    def get_terrain_params() -> dict:
        ...
        return {
            "origin": placement.origin_m,
            "scale": placement.scale_xyz,
            "yaw_deg": placement.yaw_deg,
            "mirror": placement.mirror_xy,
        }
