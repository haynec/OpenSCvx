...
from examples.rocket.senss.hop_lidar_overlay import add_body_fixed_lidar_overlay
...
def launch_viser_servers(result) -> None:
    ...
    update_lidar = add_body_fixed_lidar_overlay(
        ...
        scene_scale=SCENE_SCALE,
        dem_norm=load_senns_dem(DEM_PLACEMENT.grid),
...
def export_trajectory_rpy_csv(
    ...
    *,
    ...
    return write_trajectory_rpy_csv(
        path, np.asarray(traj["time"], dtype=np.float64), pos, rpy
    )
