...
def plot_animation(result: dict,
    ...
    for i in range(0, len(indices)-1, step):
        ...
        # Define the condition for the second order cone
        if (hasattr(params.veh, 'n_subs') and params.veh.n_subs !=0) or (hasattr(params.veh, 'covariance') and params.veh.covariance):
            A = np.diag([1 / np.tan(np.pi / params.veh.alpha_y), 1 / np.tan(np.pi / params.veh.alpha_x)])  # Conic Matrix
            z = []
            for x_val in x:
                for y_val in y:
...
def plot_main_ral_dr(result: dict,
    ...
    for i in ind[::4]:
        ...
        if result_nodal is not None:
            ...
            rotation_matrix_nodal = params.veh.qdcm(att_nodal)
        ...
        if result_nodal is not None:
            R_bi_nodal = params.veh.qdcm(drone_attitudes_nodal[indices[i]])
...
def plot_rocket_animation(result: dict,
                   params,
                   path="",
                   ) -> None:
    tof = result["tof"]
    ...
    drone_forces = result["drone_forces"]
    # subs_positions = result["sub_positions"]
    # subs_positions_est = result["sub_positions_est"]
    scp_interp_trajs = result["scp_interp"]
    scp_trajs = result["scp_trajs"]
    gates = result["gates"]\
    ...
    # Allow the plot to choose its own aspect ratio
    ...
    color_kp_est = []
    for j in range(params.vp.n_subs):
        color_kp_est.append(f'rgb({random.randint(0,255)}, {random.randint(0,255)}, {random.randint(0,255)})')
    ...
    for i in range(0, len(indices), step):
        ...
        # if params.sim.mpc:
        #     for sub_positions_est in subs_positions_est:
        #         subs_pose_est.append(sub_positions_est[indices[i]])
            
        # Convert quaternion to rotation matrix
        rotation_matrix = params.veh.qdcm(att)

        force = rotation_matrix @ drone_forces[indices[i]]
        ...
        if params.vp.tracking:    
            ...
            y = np.linspace(-5, 5, 20)
            z = np.linspace(-5, 5, 20)
        elif params.sim.model == 'rocket_6dof':
            x = np.linspace(-400, 400, 20)
            y = np.linspace(-400, 400, 20)
            z = np.linspace(-400, 400, 20)
        else:
            x = np.linspace(-30, 30, 20)
            y = np.linspace(-30, 30, 20)
            z = np.linspace(-30, 30, 20)
        ...
        # if params.scp.cov_map:
        #     # Threshold the top 10% of the cov based on z value
        #     # Assuming p is a 2D array with shape (n_points, 2)
        #     p = result['p_I']
        #     x = p[:, 0]
        #     y = p[:, 1]
        #     z = p[:, 2]
            
        #     if params.scp.cov_dyn:
        #         cov = result['drone_state'][i, cov_inds[0]:cov_inds[1]]
        #     else:
        #         from quadsim.gp import test_gp
        #         _, cov = test_gp()

        #     # Get the z values
        #     data.append(go.Scatter3d(x=x, y=y, z=z, mode='markers', marker=dict(size=10, color=cov, colorscale='Viridis')))
        ...
        for k_iter in range(3):
            if k_iter < 3:
                axis = 20 * rotated_axes[k_iter]
            color = colors[k_iter]
            label = labels[k_iter]

            if label == 'Force':
                data.append(go.Scatter3d(
                    x=[drone_positions[indices[i], 0], drone_positions[indices[i], 0] + 1E2 * force[0]],
                    y=[drone_positions[indices[i], 1], drone_positions[indices[i], 1] + 1E2 * force[1]],
                    z=[drone_positions[indices[i], 2], drone_positions[indices[i], 2] + 1E2 * force[2]],
                    mode='lines',
        ...
        for sub_pose in subs_pose:
            if params.sim.mpc:
                sub_pose_est = subs_pose_est[j]

            # Use color iter to change the color of the subject in rgb
            data.append(go.Scatter3d(x=[sub_pose[0]], y=[sub_pose[1]], z=[sub_pose[2]], mode='markers', marker=dict(size=10, color=color_kp), showlegend=False, name='Subject'))
            if params.sim.mpc:
                data.append(go.Scatter3d(x=[sub_pose_est[0]], y=[sub_pose_est[1]], z=[sub_pose_est[2]], mode='markers', marker=dict(size=10, color=color_kp_est), showlegend=False, name='Estimated Subject'))
            j += 1
        ...
        if params.vp.tracking:
            for sub_positions in subs_positions:
                sub_positions_est = subs_positions_est[sub_iter]

                data.append(go.Scatter3d(x=sub_positions[:indices[i]+1,0], y=sub_positions[:indices[i]+1,1], z=sub_positions[:indices[i]+1,2], mode='lines', line=dict(color='red', width = 10), name='Subject Position'))
                data.append(go.Scatter3d(x=sub_positions_est[:indices[i]+1, 0], y=sub_positions_est[:indices[i]+1,1], z=sub_positions_est[:indices[i]+1,2], mode='lines', line=dict(width=10, color='orange'), name='Estimated Subject Position'))

                sub_position = sub_positions[indices[i]]
                sub_position_est = sub_positions_est[indices[i]]
