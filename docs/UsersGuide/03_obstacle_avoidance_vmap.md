...
Here's a complete example with 64 spherical obstacles using a double-integrator model:
...
# States and controls (double integrator for simplicity)
position = ox.State("position", shape=(3,))
...
force.guess = np.tile([0.0, 0.0, 9.81], (n, 1))
...
# Generate 64 obstacles in a 4x4x4 grid
np.random.seed(42)
obstacle_centers = []
for i in range(4):
    for j in range(4):
        for k in range(4):
            x = -6.0 + i * 6.0 + np.random.uniform(-1.0, 1.0)
            y = -7.5 + j * 5.0 + np.random.uniform(-1.0, 1.0)
            z = 1.0 + k * 2.5 + np.random.uniform(-0.5, 0.5)
            obstacle_centers.append([x, y, z])

obstacle_centers = np.array(obstacle_centers)  # Shape: (64, 3)
obstacle_radii = np.random.uniform(1.0, 2.5, size=64)
...
# Single vectorized obstacle avoidance constraint
obstacle_avoidance = ox.ctcs(
    obstacle_radii <= ox.Vmap(
        lambda center: ox.linalg.Norm(position - center),
        batch=obstacle_centers,
